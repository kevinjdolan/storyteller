...
class BriefNormalizationError(RuntimeError):
    """Base error for brief normalization adapter failures."""
...
class GeminiBriefNormalizationAdapter:
    def __init__(
        self,
        *,
        credential: str,
    ...
    def normalize(
        ...
        log_event(
            ...
            logging.INFO,
            "ai.request.started",
            "Gemini brief normalization started.",
            operation="brief_normalization",
        ...
        try:
            ...
            log_event(
                ...
                "Gemini brief normalization succeeded.",
                operation="brief_normalization",
                model_id=self._model_id,
                prompt_version=invocation.prompt_version,
        ...
        except BriefNormalizationTransportError as exc:
            self._log_failure(
                prompt_version=invocation.prompt_version,
    ...
    def _build_retry_notifier(
        ...
        def notifier(notice: GeminiRetryNotice) -> None:
            log_event(
                ...
                "Gemini brief normalization retry scheduled.",
                operation="brief_normalization",
                model_id=self._model_id,
    ...
    def _log_failure(
        ...
        *,
        ...
        log_event(
            ...
            logging.ERROR,
            "ai.request.failed",
            "Gemini brief normalization failed.",
            operation="brief_normalization",
            model_id=self._model_id,
            prompt_version=prompt_version,
        ...
        response = self._client.post(
            ...
            json={
                ...
                "generationConfig": {
                    ...
                    "responseJsonSchema": get_brief_normalization_response_schema(),
                },
            },
        )
...
def build_brief_normalization_invocation(
    context: BriefNormalizationPromptContext,
    *,
    model_id: str,
) -> BriefNormalizationInvocation:
    return BriefNormalizationInvocation(
        prompt_version=BRIEF_NORMALIZATION_PROMPT_VERSION,
        model_id=model_id,
        context=context,
        rendered_prompt=render_brief_normalization_prompt(context),
    )


@lru_cache(maxsize=1)
def get_brief_normalization_response_schema() -> dict[str, Any]:
    return _sanitize_json_schema(BriefNormalizationStructuredOutput.model_json_schema())


@lru_cache(maxsize=1)
def _read_prompt_template() -> str:
    return (PROMPTS_ROOT / "brief_normalization.md").read_text(encoding="utf-8")
